...
/script UIErrorsFrame:Clear();
-- ATK
/run if AmIBehind==true then cst("Полоснуть") end
/run if range("Звериная атака - кошка",t)==1 and cd("Звериная атака - кошка")==0 then cst("Звериная атака - кошка") end
/script UIErrorsFrame:Clear();
/run if range("Цапнуть",t)==1 and cd("Тигриное неистовство")==0 then cst("Тигриное неистовство") end
/script UIErrorsFrame:Clear();
/run if range("Разорвать")==1 and GetComboPoints(p)>2  and not dbuf(t,"Разорвать") then cst("Разорвать") end
/script UIErrorsFrame:Clear();
/run if range("Глубокая рана")==1 and not dbuf(t,"Глубокая рана") then cst("Глубокая рана") end
...
-- END
---------------------------------20/05/2015
-- Работающий behindCheck
/run if not behindCheck then behindCheck = CreateFrame("Frame")end
/run local frame = behindCheck frame:RegisterEvent("UI_ERROR_MESSAGE") frame:SetScript("OnEvent", function(self, event, ...)if (select(1,...):find("позади") ~= nil) then CastSpellByName("Цапнуть") end end)
/run CastSpellByName("Полоснуть");
-- END
